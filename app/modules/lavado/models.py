...
class HistorialLavado(Base):
    ...
    id_historial = Column(UUID(as_uuid=True),
                          primary_key=True, default=uuid.uuid4, index=True)
    id_orden = Column(UUID(as_uuid=True), ForeignKey(
        "orden_servicio.id_orden"))
    id_tipo_lavado = Column(UUID(as_uuid=True), ForeignKey(
        "tipo_lavado.id_tipo_lavado"))
    fecha_lavado = Column(Date, nullable=False)
    observaciones = Column(String, nullable=True)
    # iniciado, terminado, inspeccionado
    estado_lavado = Column(String, nullable=False)
